...
class SupportChatbot:
        ...
        self.config = {
            ...
            "vector_store": {
                ...
                "config": {
                    ...
                    "host": os.environ["LINDORM_HOST"],
                    "port": 30070,
                    "user": os.environ["LINDORM_USER"],
                    "password": os.environ["LINDORM_PASSWORD"],
                }
            },
            "embedder": {
                "provider": "openai",
    ...
    def handle_customer_query(self, user_id: str, query: str) -> str:
        ...
        fetched_results = self.get_relevant_history(user_id, query)
        relevant_history = fetched_results.get("results", [])
        relevant_relationships = fetched_results.get("relations", [])
        ...
        if relevant_relationships:
            context += "The already known Relationships:\n"
            for relationships in relevant_relationships:
                context += f"{relationships['source']} {relationships['relationship']} {relationships['destination']}\n"
                context += "---\n"

        else:
            if not relevant_relationships and not relevant_history:
